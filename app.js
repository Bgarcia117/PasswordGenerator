...
function generatePassword(upper, lower, number, symbol, length){
    ...
    // The filter method creates an array with all the elements that pass the test implemented by the provided function
    typesArr.filter(item => {
        console.log(item[1]);
        return(item[1]);
    });

    // 3. Loop over the legnth and call the generator function for each type
    // Building a password with a for loop
    for (i = 0; i < length; i+= typesCount){
        typesArr.forEach(type => {
            const funName = type[0];
...
generateEl.addEventListener(`click`, () => {
    ...
    const legnth = parseInt(lengthEl.value);
    length.value;
